...
interface EmitterFormProps {
  isRTL: boolean;
  initialData?: Emitter;
  onSubmit: (data: Partial<Emitter>) => Promise<void>;
  onCancel: () => void;
}

export function EmitterForm({ isRTL, initialData, onSubmit, onCancel }: EmitterFormProps) {
  const [formData, setFormData] = useState<Partial<Emitter>>(initialData || {
    name: { fr: '', ar: '' },
    address: { fr: '', ar: '' }
  });
